        ...
        <div class="col-md-12">
            <div class="card shadow">
                <div class="card-header py-2">
                    ...
                    <a href="javascript:void(0)" data-link="{{ route('speed_loss.store') }}" data-action="add"
                    ...
                    <ul class="tree">
                        @foreach ($data as $key => $item)
                            @if (count((array) $item->children) > 0)
                                <li id="menu{{ $key + 1 }}">
                                    <label for="menu{{ $key + 1 }}">
                                        <a class="float-left">{{ $item->jenis_speedloss }}</a>

                                        <a href="javascript:void(0)" class="float-right">
                                            <span class="badge badge-danger btn-action"
                                                data-link="{{ route('speed_loss.store') }}" data-type="child"
                            ...
                            @else
                                <li id="menu{{ $key + 1 }}">
                                    <a>
                                        <label for="menu{{ $key + 1 }}">{{ $item->jenis_speedloss }}</label>
                                        <input checked="" id="menu{{ $key + 1 }}" value="" type="checkbox">

                                        <span class="badge badge-primary btn-action" style="cursor: pointer;"
                                            data-link="{{ route('speed_loss.store') }}" data-action="add" data-type="child"
                                            data-id_sub="{{ $item->id }}">
                                            <i class="fas fa-plus"></i>
                                        </span>
                                        <span class="badge badge-success btn-action" style="cursor: pointer;"
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="form-group row">
                                    <label for="jenis_speedloss" class="col-md-5">Speed Loss Name<span
                                            style="color: red;font-weight: bold;">*</span></label>
                                    <input type="text" required autocomplete="off"
                                        class="form-control col-md-7 @error('jenis_speedloss') is-invalid @enderror"
                                        name="jenis_speedloss" id="jenis_speedloss">
                                    @error('jenis_speedloss')
        ...
        $(function() {
            ...
            $(document).on("click", ".btn-action", function(e) {
                ...
                if (action == 'add') {
                    ...
                    if (type == 'parent') {
                        $(".form-modal").find(".modal-title").html("Form Add Parent Speed Loss");
                    } else {
                        $(".form-modal").find(".modal-title").html("Form Add Child Speed Loss");
                    }
                    ...
                    $('.save-form').safeform({
                        ...
                        submit: function() {
                            ...
                            let jenis_speedloss = $(this).find("input[name=jenis_speedloss]")
                                .val();
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ...
                                    id_sub: id_sub,
                                    type: type,
                                    jenis_speedloss: jenis_speedloss,
                                    desc: desc,
                                },
                                dataType: "json",
                    ...
                    let url = '{{ route('speed_loss.show', ':id') }}';
                    url = url.replace(':id', `${id_sub}`);

                    $('.form-modal').modal('show');

                    $(".form-modal").find(".modal-title").html("Form Edit Speed Loss");
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            $(`input[name=jenis_speedloss]`).val(response.jenis_speedloss);
                            ...
                            $('.save-form').safeform({
                                ...
                                submit: function(e) {
                                    // put here validation and ajax stuff...
                                    let jenis_speedloss = $(this).find(
                                            "input[name=jenis_speedloss]")
                                        .val();
                                    let id = $(this).find(
                                            "input[name=id_sub]")
                                        .val();
